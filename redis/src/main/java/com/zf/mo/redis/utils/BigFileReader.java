...
public class BigFileReader {
    ...
    public void start(){
        long ererySize = this.fileLength/this.threadSize;
        try {
            calculateStartEnd(0,ererySize);
        } catch (IOException e) {
            e.printStackTrace();
            return;
        }
    ...
    public static void main(String[] args) {
       ...
       /* BigFileReader.Builder builder = new BigFileReader.Builder("C:\\Users\\zhouf\\Desktop\\yj.txt", new IHandle() {
            @Override
            public void handle(String line) {
                System.out.println("line:"+line);
            }
        });
        builder.withBufferSize(1024*1024)
                .withCharset("UTF-8")
                .withThreadSize(10);
        BigFileReader build = builder.build();
        build.start();*/
    //=================================================================


        try {
            RandomAccessFile accessFile = new RandomAccessFile("C:\\\\Users\\\\zhouf\\\\Desktop\\\\yj.txt", "r");
            MappedByteBuffer mappedByteBuffer = accessFile.getChannel().map(FileChannel.MapMode.READ_ONLY, 0, 1024);
            byte[] bytes = new byte[1024];
            mappedByteBuffer.get(bytes, 0, 1024);
